...
import { handleLiquidityAdded, handleLiquidityRemoved } from "../src/tlp"
import { createLiquidityAddedEvent, createLiquidityRemovedEvent } from "./tlp-utils"
...
describe("liquidityAdded", () => {
    ...
    beforeAll(() => {
        let liquidityAddedEvent = createLiquidityAddedEvent(
            ...
            BigInt.fromI64(xDeposit),
            BigInt.fromI64(yDeposit)
        )
    ...
    test("populates fields correctly", () => {
        ...
        assert.fieldEquals(entityName, id, "xDeposit", xDeposit.toString())
        assert.fieldEquals(entityName, id, "yDeposit", yDeposit.toString())
    ...
    test("adds liquidity if already existant to liquidity position", () => {
        ...
        assert.fieldEquals(entityName, id, "xDeposit", (xDeposit * 2).toString())
        assert.fieldEquals(entityName, id, "yDeposit", (yDeposit * 2).toString())
    })
})
